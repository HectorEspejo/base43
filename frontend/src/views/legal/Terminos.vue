<template>
  <div class="container-custom mx-auto section-padding py-8">
    <div class="max-w-4xl mx-auto">
      <h1 class="text-4xl font-bold mb-8">Términos y Condiciones de Uso</h1>
      
      <div class="prose prose-lg max-w-none">
        <p class="text-gray-600 mb-6">Última actualización: {{ currentDate }}</p>
        
        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">1. Aceptación de los Términos</h2>
          <p>
            Al acceder y utilizar el sitio web de Base43 ("el Sitio"), usted acepta estar sujeto a estos Términos y Condiciones de Uso, 
            todas las leyes y regulaciones aplicables, y acepta que es responsable del cumplimiento de las leyes locales aplicables. 
            Si no está de acuerdo con alguno de estos términos, tiene prohibido usar o acceder a este sitio.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">2. Descripción del Servicio</h2>
          <p>
            Base43 es una plataforma web moderna que proporciona funcionalidades completas para proyectos web. Proporcionamos:
          </p>
          <ul class="list-disc pl-6 mt-3">
            <li>Información sobre proyectos de vivienda colaborativa</li>
            <li>Recursos y documentación sobre cohousing</li>
            <li>Herramientas de comunicación entre usuarios registrados</li>
            <li>Directorio de partners y colaboradores</li>
            <li>Noticias y actualizaciones del sector</li>
          </ul>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">3. Uso de la Plataforma</h2>
          <h3 class="text-xl font-semibold mb-2">3.1 Registro de Usuario</h3>
          <p>
            Para acceder a ciertas funcionalidades, puede ser necesario registrarse. Usted se compromete a:
          </p>
          <ul class="list-disc pl-6 mt-3">
            <li>Proporcionar información veraz, precisa y completa</li>
            <li>Mantener la seguridad de su cuenta y contraseña</li>
            <li>Notificar inmediatamente cualquier uso no autorizado de su cuenta</li>
            <li>Asumir la responsabilidad de todas las actividades bajo su cuenta</li>
          </ul>

          <h3 class="text-xl font-semibold mb-2 mt-4">3.2 Conducta del Usuario</h3>
          <p>
            Los usuarios se comprometen a NO:
          </p>
          <ul class="list-disc pl-6 mt-3">
            <li>Utilizar el Sitio para fines ilegales o no autorizados</li>
            <li>Publicar contenido ofensivo, difamatorio o inapropiado</li>
            <li>Interferir con el funcionamiento del Sitio</li>
            <li>Intentar acceder a áreas restringidas sin autorización</li>
            <li>Recopilar información de otros usuarios sin consentimiento</li>
          </ul>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">4. Propiedad Intelectual</h2>
          <p>
            Todo el contenido del Sitio, incluyendo textos, gráficos, logos, imágenes y software, es propiedad de Base43 
            o sus proveedores de contenido y está protegido por las leyes de propiedad intelectual. 
            Base43 opera bajo licencia MIT, promoviendo el uso compartido y colaborativo del código.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">5. Contenido del Usuario</h2>
          <p>
            Al publicar contenido en el Sitio, usted:
          </p>
          <ul class="list-disc pl-6 mt-3">
            <li>Conserva sus derechos de propiedad intelectual</li>
            <li>Otorga a Base43 una licencia no exclusiva para usar, mostrar y distribuir dicho contenido</li>
            <li>Garantiza que tiene los derechos necesarios sobre el contenido publicado</li>
            <li>Acepta que otros usuarios puedan ver y compartir su contenido según los principios de la plataforma</li>
          </ul>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">6. Privacidad y Protección de Datos</h2>
          <p>
            El uso de nuestro Sitio está sujeto a nuestra Política de Privacidad. 
            Al utilizar el Sitio, usted acepta las prácticas descritas en dicha política.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">7. Limitación de Responsabilidad</h2>
          <p>
            Base43 no será responsable de:
          </p>
          <ul class="list-disc pl-6 mt-3">
            <li>Daños indirectos, incidentales o consecuentes</li>
            <li>Pérdida de datos o interrupción del servicio</li>
            <li>Acciones de terceros o contenido de usuarios</li>
            <li>Decisiones tomadas basándose en información del Sitio</li>
          </ul>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">8. Indemnización</h2>
          <p>
            Usted acepta indemnizar y eximir de responsabilidad a Base43, sus directores, empleados y agentes 
            de cualquier reclamación o demanda derivada de su uso del Sitio o violación de estos términos.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">9. Modificaciones</h2>
          <p>
            Base43 se reserva el derecho de modificar estos términos en cualquier momento. 
            Las modificaciones entrarán en vigor inmediatamente después de su publicación en el Sitio. 
            Su uso continuado del Sitio constituye la aceptación de los términos modificados.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">10. Terminación</h2>
          <p>
            Base43 puede terminar o suspender su acceso al Sitio inmediatamente, 
            sin previo aviso, por cualquier motivo, incluyendo el incumplimiento de estos Términos.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">11. Ley Aplicable</h2>
          <p>
            Estos términos se regirán e interpretarán de acuerdo con las leyes de España, 
            sin tener en cuenta sus disposiciones sobre conflictos de leyes.
          </p>
        </section>

        <section class="mb-8">
          <h2 class="text-2xl font-semibold mb-4">12. Contacto</h2>
          <p>
            Si tiene preguntas sobre estos Términos y Condiciones, puede contactarnos en:
          </p>
          <div class="mt-4 bg-gray-100 p-4 rounded-lg">
            <p><strong>Base43</strong></p>
            <p>Email: contacto@base43.org</p>
          </div>
        </section>
      </div>
    </div>
  </div>
</template>

<script setup>
import { computed } from 'vue'

const currentDate = computed(() => {
  const date = new Date()
  return date.toLocaleDateString('es-ES', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  })
})
</script>